...
export const ChartSkeleton: React.FC<ChartSkeletonProps> = ({
  ...
  // For quad layout (2x2 grid)
  if (type === "quad") {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Four Chart Skeletons */}
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="bg-white p-6 rounded-lg shadow-sm">
            <div className="h-8 w-40 bg-gray-200 rounded animate-pulse mb-4"></div>
            <div
              className={`min-h-[${height}px] bg-gray-100 rounded animate-pulse flex items-center justify-center`}
            >
  ...
  // For double layout (2 charts side by side)
  if (type === "double") {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Two Chart Skeletons */}
