...
export function DeviceDetailsModal({
  ...
  const [deviceDetails, setDeviceDetails] = useState<DeviceResponse | null>(null);
  ...
  const getChartData = () => {
    ...
    if (data.type === "EMISSION" && deviceData.emissionData) {
      labels.push(...deviceData.emissionData.map(item => 
      ...
      labels.push(...deviceData.gpsData.map(item => 
        format(new Date(item.timestamp), 'MMM dd, HH:mm')
      ));
      ...
      labels.push(...deviceData.fuelData.map(item => 
        format(new Date(item.timestamp), 'MMM dd, HH:mm')
      ));
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: deviceDetails?.data.type === 'EMISSION' 
            ...
            <TableBody>
              {deviceData.gpsData.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    <Badge variant={item.trackingStatus ? "default" : "secondary"}>
    ...
    return (
      <div className="flex items-center justify-between py-4">
        <div className="text-sm text-muted-foreground">
          Showing {Math.min(limit, total - (page - 1) * limit)} of {total} entries
        </div>
