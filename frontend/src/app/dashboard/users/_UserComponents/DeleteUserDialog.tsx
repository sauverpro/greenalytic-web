...
import { hardDeleteUser, softDeleteUser } from "../_UserActions/actions" // 👈 import your actions
  ...
  const handleDelete = async () => {
    ...
    try {
      if (deletionType === "hard") {
        await hardDeleteUser(String(user.id))
      } else {
        await softDeleteUser(String(user.id))
      }

      toast.success(`User ${deletionType === "hard" ? "permanently" : "temporarily"} deleted`)
      onDeleted?.()
      onOpenChange(false)
      setConfirmText("")
    } catch (error: any) {
        ...
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {deletionType === "hard"
              ...
              : "This will deactivate the user but keep their data."}
          </AlertDialogDescription>
        </AlertDialogHeader>
